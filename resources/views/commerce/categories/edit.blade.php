        ...
        <div class="glass-card-solid p-6">
            <form action="{{ route('categories.update', $category) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" id="name" class="form-input @error('name') border-red-500 @enderror" value="{{ old('name', $category->name) }}" required onkeyup="generateSlug(this.value)">
                    @error('name')<p class="form-error">{{ $message }}</p>@enderror
                </div>

                <div class="form-group">
                    <label for="slug" class="form-label">Slug (URL)</label>
                    <input type="text" name="slug" id="slug" class="form-input bg-gray-50 @error('slug') border-red-500 @enderror" value="{{ old('slug', $category->slug) }}" required readonly>
                    @error('slug')<p class="form-error">{{ $message }}</p>@enderror
                </div>

                <div class="form-group">
                    <label for="icon" class="form-label">Icon (Emoji)</label>
                    <input type="text" name="icon" id="icon" class="form-input" value="{{ old('icon', $category->icon) }}" placeholder="Contoh: 📦">
                </div>
