...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <form action="{{ route('purchases.update', $purchase) }}" method="POST" id="purchaseForm" class="space-y-4" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="glass-card p-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="text" name="reference_number" class="form-input bg-gray-100 cursor-not-allowed" value="{{ old('reference_number', $purchase->reference_number) }}" readonly>
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-100 dark:border-gray-700">
                ...
                <div>
                    ...
                    <textarea name="note" rows="1" class="form-input resize-none" placeholder="Catatan tambahan (Opsional)...">{{ old('note', $purchase->note) }}</textarea>
                </div>
                <div>
                    ...
                    @if($purchase->receipt_image)
                        <div class="mb-2">
                            <a href="{{ \Storage::url($purchase->receipt_image) }}" target="_blank" class="text-primary-600 hover:underline text-xs flex items-center gap-1">
                    ...
                    <p class="text-[10px] text-gray-500 mt-1">Format: JPG, PNG. Max: 2MB. (Upload baru akan mengganti yang lama)</p>
                </div>
            </div>
        </div>
...
<script>
    ...
    document.getElementById('purchaseForm').addEventListener('submit', function(e) {
        ...
        // Default confirm for completed status
        @if($purchase->status === 'completed')
            if (!confirm('Apakah Anda yakin ingin menyimpan perubahan? Data stok dan akuntansi akan dihitung ulang.')) {
                e.preventDefault();
            }
        @endif
    });
