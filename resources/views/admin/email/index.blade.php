...
@php
    ...
    $currentFolder = $folders[$activeFolder] ?? $folders['inbox'];
@endphp
...
<style>
    ...
    .folder-tabs {
        ...
        background: linear-gradient(135deg, {{ $currentFolder['color'] }}11, {{ $currentFolder['color'] }}08);
    ...
    .folder-tab:hover { background: #f3f4f6; color: #374151; }
    .folder-tab.active {
        background: #fff; color: {{ $currentFolder['color'] }};
        font-weight: 600;
        border-color: #e5e7eb; border-bottom-color: #fff;
        margin-bottom: -1px; z-index: 1;
    }
    .folder-tab svg { width: 16px; height: 16px; }
    .folder-tab.active svg { color: {{ $currentFolder['color'] }}; }

    /* Compose Button */
    .compose-btn {
        display: flex; align-items: center; gap: 8px;
        margin: 12px; padding: 12px 20px;
        background: linear-gradient(135deg, {{ $currentFolder['color'] }}, {{ $currentFolder['color'] }}dd);
        color: #fff; border-radius: 12px; font-size: 14px;
        font-weight: 600; text-decoration: none;
        box-shadow: 0 2px 8px {{ $currentFolder['color'] }}40;
        transition: all 0.2s; flex-shrink: 0;
    }
    .compose-btn:hover {
        transform: translateY(-1px);
        box-shadow: 0 4px 16px {{ $currentFolder['color'] }}50;
    }
    ...
    .email-search-input:focus {
        ...
        box-shadow: 0 0 0 3px {{ $currentFolder['color'] }}18;
    }
    ...
    .email-list-item:hover { background: {{ $currentFolder['bg'] }}; }
    .email-list-item.active {
        background: {{ $currentFolder['bg'] }};
        border-left-color: {{ $currentFolder['color'] }};
    }
    ...
    .email-date.bold { font-weight: 700; color: {{ $currentFolder['color'] }}; }
    ...
    .email-empty-icon svg { width: 36px; height: 36px; color: {{ $currentFolder['color'] }}; }
    ...
    .toolbar-btn:hover { background: {{ $currentFolder['bg'] }}; color: {{ $currentFolder['color'] }}; }
    ...
    .reading-subject {
        ...
        padding: 28px 32px 16px; line-height: 1.3;
    }
    ...
    .reading-attachments {
        ...
        background: {{ $currentFolder['bg'] }}; border: 1px solid {{ $currentFolder['color'] }}22;
        border-radius: 12px;
    }
    .reading-attachments h4 { font-size: 11px; font-weight: 700; text-transform: uppercase; color: {{ $currentFolder['color'] }}; letter-spacing: 0.05em; margin-bottom: 12px; }
    ...
    .reading-attachment-item:hover { border-color: {{ $currentFolder['color'] }}; }
    ...
    /* Quick Reply Box */
    .quick-reply {
        margin: 0 32px 32px; border: 1px solid #e5e7eb; border-radius: 12px;
        overflow: hidden; transition: all 0.2s;
    }
    .quick-reply:hover { border-color: {{ $currentFolder['color'] }}; box-shadow: 0 2px 8px {{ $currentFolder['color'] }}15; }
    .quick-reply a {
        display: flex; align-items: center; gap: 10px;
        padding: 16px 20px; text-decoration: none; color: #9ca3af;
        font-size: 14px; transition: all 0.15s;
    }
    .quick-reply a:hover { background: {{ $currentFolder['bg'] }}; color: {{ $currentFolder['color'] }}; }
    ...
    .btn-send:hover { transform: translateY(-1px); box-shadow: 0 4px 12px {{ $currentFolder['color'] }}50; }
    ...
    /* Alerts */
    .email-alert { padding: 10px 16px; font-size: 13px; font-weight: 500; border-bottom: 1px solid transparent; }
    ...
    .dark .folder-tabs { background: {{ $currentFolder['color'] }}08; border-color: #374151; }
    .dark .folder-tab { color: #9ca3af; }
    .dark .folder-tab:hover { background: #1f2937; color: #f3f4f6; }
    .dark .folder-tab.active { background: #111827; border-color: #374151; }
    .dark .email-list-item { border-color: #1f2937; }
    .dark .email-list-item:hover { background: {{ $currentFolder['color'] }}10; }
    .dark .email-list-item.active { background: {{ $currentFolder['color'] }}15; }
    ...
    <div class="email-list-pane {{ isset($selectedMessage) ? 'has-selected' : '' }} {{ isset($composeMode) ? 'has-compose' : '' }}">
        ...
        <div class="folder-tabs">
            @foreach($folders as $slug => $info)
                <a href="{{ route('admin.email.folder', $slug) }}" 
                   ...
                   style="{{ $activeFolder === $slug ? 'color:' . $info['color'] . ';' : '' }}">
        ...
        <div class="email-search-bar">
            <div style="position:relative;">
                <svg style="position:absolute;left:12px;top:50%;transform:translateY(-50%);width:16px;height:16px;color:#9ca3af;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                <input type="text" placeholder="Cari di {{ $currentFolder['label'] }}..." class="email-search-input">
            </div>
        </div>
            ...
            @elseif(!isset($imapError))
                <div class="email-empty-state">
                    <div class="email-empty-icon">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $currentFolder['icon'] }}"></path></svg>
                    </div>
                    <h3>{{ $currentFolder['label'] }} Kosong</h3>
