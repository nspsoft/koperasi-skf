<section>
    ...
    <!-- Points Summary Card -->
    <div class="mt-6 mb-6 overflow-hidden rounded-2xl shadow-lg border border-gray-100 dark:border-gray-800">
        <div class="bg-gradient-to-br from-indigo-600 to-purple-700 p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-indigo-100 text-xs font-bold uppercase tracking-wider mb-1">Total Poin Terkumpul</p>
                    <div class="flex items-baseline gap-2">
                        <p class="text-4xl font-extrabold">{{ number_format($user->points) }}</p>
                        <p class="text-indigo-200 text-sm">≈ Rp {{ number_format($user->points_value) }}</p>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-2 bg-white dark:bg-gray-800 divide-x divide-gray-100 dark:divide-gray-700">
            <div class="p-4 text-center">
                <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">Poin Belanja</p>
                <p class="text-xl font-bold text-gray-900 dark:text-white">{{ number_format($loyaltyPoints ?? 0) }}</p>
            </div>
            <div class="p-4 text-center">
                <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">Poin Performa</p>
                <p class="text-xl font-bold text-gray-900 dark:text-white">{{ number_format($performancePoints ?? 0) }}</p>
            </div>
        </div>
    </div>
        ...
        <div class="mt-6 overflow-hidden border border-gray-200 dark:border-gray-700 rounded-xl">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @php $hasPerformance = false; @endphp
                    @foreach($performanceHistories as $history)
                    @if($history->type !== 'loyalty')
                    @php $hasPerformance = true; @endphp
                    ...
                    @if(!$hasPerformance)
                    <tr>
                        <td colspan="4" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400 text-xs">
                            Tidak ada riwayat poin performa (manual)
                        </td>
                    </tr>
                    @endif
                </tbody>
