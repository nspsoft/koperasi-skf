...
// Find required accounts for journal entries if accounting module exists
...
try {
    for ($month = 1; $month <= 6; $month++) {
        $date = Carbon::create(2026, $month, rand(5, 25));
        ...
        foreach ($members as $member) {
            ...
            // Create Journal if accounts exist
            if ($cashAccount && $savingWajibAccount) {
                ...
                JournalEntryLine::create([
                    'journal_entry_id' => $journal->id,
                    'account_id' => $savingWajibAccount->id,
                    'debit' => 0,
                    'credit' => $savingWajib->amount,
                    'description' => $savingWajib->description,
                ]);
            }

            // Simpanan Sukarela (Randomly for some members)
            if (rand(1, 10) <= 4) { // 40% chance
                ...
                if ($cashAccount && $savingSukarelaAccount) {
                    ...
                    JournalEntryLine::create([
                        'journal_entry_id' => $journal->id,
                        'account_id' => $savingSukarelaAccount->id,
                        'debit' => 0,
                        'credit' => $savingSukarela->amount,
                        'description' => $savingSukarela->description,
                    ]);
                }
            }
    ...
    DB::commit();
    echo "\n\n>>> BERHASIL! DATA SIMPANAN JAN-JUN TELAH DITAMBAHKAN <<<\n\n";
} catch (\Exception $e) {
    DB::rollBack();
