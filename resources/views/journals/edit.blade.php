...
<div class="page-header">
    <div>
        ...
        <p class="page-subtitle">Koreksi akun, nilai debit-kredit, dan keterangan jurnal manual</p>
    </div>
    <div>
...
<form action="{{ route('journals.update', $journal) }}" method="POST" x-data="journalForm()">
    @csrf
    @method('PUT')
            ...
            <div class="glass-card-solid p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="transaction_date" class="form-input" value="{{ old('transaction_date', $journal->transaction_date->format('Y-m-d')) }}" required>
                    </div>
                    <div>
                        <label class="form-label">{{ __('messages.journals_page.table_desc') }}</label>
                        <input type="text" name="description" class="form-input" value="{{ old('description', $journal->description) }}" placeholder="{{ __('messages.journals_page.desc_placeholder') }}" required>
                    </div>
                </div>
            </div>
        </div>
...
function journalForm() {
    return {
        lines: {!! json_encode($journal->lines->map(function($line) {
            return [
